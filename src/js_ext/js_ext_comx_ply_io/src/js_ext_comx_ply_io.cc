...
std::vector<string> splitAndConvertToInt(const std::string& input,
    ...
    // Use std::getline to split the string.
...
JS_EXT_FUNC_BEGIN(loadFilesModel, 4, LOADFILESMODEL_FUNC_USAGE) {
    ...
    // 加载并缓存STL文件
    {
        ...
        if (!file.good()) {
            cout << "[ERROR] STL file does not exist: " << stlPath << endl;
        }
        else {
            file.close();
            ...
            try {
                ...
                if (stlOutput &&
                    ...
                    cout << "[ERROR] Failed to read STL file: "
                        << stlPath << endl;
                }
            }
            catch (const std::exception& e) {
                cout << "[ERROR] Exception while reading STL file: " << e.what()
                    << endl;
            }
        }
    }
...
bool createHeatFluxes(double x, double y, double z, double nx, double ny, double nz,
    ...
    setHeatFlux(x, y, z, nx, ny, nz, name, work, scale);
...
bool createConvections(double x, double y, double z, double nx, double ny,
    ...
    setConvection(x, y, z, nx, ny, nz, name, work, scale);
...
bool CreateThermalConstraints(double x, double y, double z, double xn, double yn, double zn, string name, vtkRenderer* renderer, workData* work, string id, double scale)
{
    ...
    CreateThermalConstraint(x, y, z, xn, yn, zn, name, renderer, work, scale);
...
void OnOpenMesh(Point3D startPoint, Point3D endPoint, double numXSegments,
    ...
    work->actor_Mesh->GetProperty()->SetColor(
        1.0, 1.0, 1.0); // Set mesh line color.
    renderer->AddActor(work->actor_Mesh);
}
...
void rendererPly(vtkRenderer* renderer, postManage* work, TPlyEntry* pEntry, int isPointMode) {
    ...
    bool hasVonMises = (pEntry->DisTotal.size() == numNodes);

    for (size_t i = 0; i < numNodes; i++) {
        double val = hasVonMises ? pEntry->DisTotal[i] : 0.0;
        // 传入占位法线 (0,0,1)
        inputMesh.addNode(
            pEntry->points[i * 3],
            pEntry->points[i * 3 + 1],
    ...
    // =========================================================
        ...
        // 【修改】为 PointData 分配空间
        work->plyVonmises->SetName("DisTotal");
        ...
        work->plyData->GetPointData()->SetActiveScalars("DisTotal");
        ...
        // *************************************************************
        std::cout << "[DEBUG] Use cell rendering." << std::endl;
        ...
        work->plyData->GetCellData()->SetActiveScalars("DisTotal");
